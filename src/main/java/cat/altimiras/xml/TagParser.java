			...
			while (cursor < xml.length) {
				...
				if (in) {
					if (val == '>') { //detecting close tag definition
						if (name == null) {
							name = new byte[bufferCursor];
							System.arraycopy(buffer, 0, name,0,bufferCursor);
					...
					if (val == ' ' && quotes != 1) {
						...
						if (attribute != null && quotes == 2) {
							attribute.value = new byte[bufferCursor];
							System.arraycopy(buffer, 0, attribute.value,0,bufferCursor);
						...
						attribute.name = new byte[bufferCursor];
						System.arraycopy(buffer, 0, attribute.name,0,bufferCursor);
						...
						if (namespace == null) {
							namespace = new byte[bufferCursor];
							System.arraycopy(buffer, 0, namespace,0,bufferCursor);
